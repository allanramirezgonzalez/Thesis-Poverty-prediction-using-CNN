import os
os.environ["CUDA_VISIBLE_DEVICES"]="3"

#==============================================================
#import modules
import numpy as np
import pandas as pd
import random
import cv2

import tensorflow as tf
from tensorflow.keras.models import Sequential
from tensorflow.keras import models, layers
from tensorflow.keras import losses
from tensorflow.keras import optimizers
from tensorflow.keras.callbacks import Callback, EarlyStopping, ReduceLROnPlateau
from tensorflow.keras.layers import Dropout, Dense, GlobalAveragePooling2D
from tensorflow.keras.utils import Sequence
from tensorflow.keras.applications import EfficientNetB3

import albumentations as albu

#==============================================================
#load EfficientNet-B3 model pre-trained with imagenet
height, width, channels = 224, 224, 6

cnn_source = EfficientNetB3(weights='imagenet', include_top=False, input_shape=(width, height, 3))

conv_base = EfficientNetB3(weights=None, include_top=False, input_shape=(width, height, 6))

penalty = 0.1 
regularizer = tf.keras.regularizers.l2(penalty)

for i in np.arange(5,len(conv_base.layers)):
    layer_source = cnn_source.get_layer(index=i)
    layer_dest = conv_base.get_layer(index=i)
    w = layer_source.get_weights()
    layer_dest.set_weights(w)
    for attr in ['kernel_regularizer']:
        if hasattr(layer_dest, attr):
            setattr(layer_dest, attr, regularizer)

#==============================================================
#add layer to have a single output
model = Sequential()
model.add( conv_base )
model.add( GlobalAveragePooling2D() )
model.add( Dropout(0.5) )
model.add( Dense(1, activation='sigmoid') )

print('load model - ok')

#==============================================================
#load vulnerability indices from 2010
filename = '../data/residential_blocks_data2010.csv'
df = pd.read_csv(filename)
vi2010 = df['v_index']

print('load vulnerability data - ok')

#==============================================================
#load partition
path = '../image_partition/'

ind_train = np.load(path + 'ind_train.npy')
ind_val = np.load(path + 'ind_val.npy')

x_train = np.load(path + 'x_train.npy')
x_val = np.load(path + 'x_val.npy')

y_train = np.array(vi2010[ind_train])
y_val = np.array(vi2010[ind_val])

print('load partition - ok')

#==============================================================
batch_size = 32

def resize_img(img, shape):
    return cv2.resize(img, (shape[1], shape[0]), interpolation=cv2.INTER_CUBIC)

#==============================================================
#data generator for training and validation
random.seed(0)

class DataGenerator(Sequence):
    def __init__(self, x_set, y_set, batch_size=batch_size, dim=(height, width), channels=channels, augment=False):
        self.x, self.y = x_set, y_set
        self.batch_size = batch_size
        self.dim = dim
        self.channels = channels
        self.indexes = np.arange(0, len(self.x))
        self.augment = augment
        
    def __on_epoch_end(self):
        self.indexes = np.arange(0, len(self.x))
        np.random.shuffle(self.indexes)

    def __len__(self):
        return int(np.ceil(len(self.x) / float(self.batch_size)))

    def __getitem__(self, idx):
        batch_indexes = self.indexes[idx * self.batch_size:(idx+1) * self.batch_size]
        
        #generate mini-batch of X
        #X = np.empty((self.batch_size, *self.dim, self.channels))
        X = []
        for i, ID in enumerate(batch_indexes):
            img = self.x[ID]
            img = resize_img(img, self.dim)
            X.append(img)
        X = np.array(X)
        #generate mini-batch of y
        y = self.y[batch_indexes]
        
        #augmentation on the training dataset
        if self.augment==True:
            X = self.__augment_batch(X)
        return X, y
    
    #augmentation for one image
    def __random_transform(self, img):
        composition = albu.Compose([albu.HorizontalFlip(p=0.5),
                                    albu.VerticalFlip(p=0.5),
                                    albu.GridDistortion(p=0.2),
                                    albu.ElasticTransform(p=0.2)])
        return composition(image=img)['image']
    
    #augmentation for batch of images
    def __augment_batch(self, img_batch):
        for i in np.arange(0, len(img_batch)):
            img_batch[i] = self.__random_transform(img_batch[i])
        return img_batch
    

train_gen = DataGenerator(x_train, y_train, augment=True)
val_gen = DataGenerator(x_val, y_val, augment=False)

#==============================================================
#compile the model
learning_rate = 1.0e-4
mse = tf.keras.losses.MeanSquaredError()
opt = tf.keras.optimizers.Adam(learning_rate=learning_rate)

#early stopping to monitor the validation loss and avoid overfitting
early_stop = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=3, restore_best_weights=True)

#reducing learning rate on plateau
rlrop = ReduceLROnPlateau(monitor='val_loss', mode='min', patience=3, factor= 0.5, min_lr= 1e-6, verbose=1)

model.compile(optimizer=opt, loss=mse)

print('compile model - ok')

#==============================================================
#fit the model
epochs = 10
res = model.fit(train_gen,
                validation_data=val_gen,
                callbacks = [early_stop, rlrop],
                epochs=epochs, verbose=1)

print('train model - ok')

#==============================================================
#save training history
filename = 'training_history_efnb3_regression_model_b.csv'
train_loss = res.history['loss']
val_loss = res.history['val_loss']
df = pd.DataFrame({'train_loss':train_loss, 'val_loss':val_loss})
df.to_csv(filename)

#==============================================================
#saving the trained model
model.save('model_efnb3_regression_model_b.h5')

print('save model - ok')

